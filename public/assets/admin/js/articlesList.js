layui.config({
  ...
  //       },2000);
	...
	$("body").on("click",".news_edit",function(){  //编辑
		...
		location.href="/admin/article/edit/"+aid;
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td align="left">'+currData[i].article_title+'</td>'
						+'<td align="left">'+currData[i].cname+'</td>'
			    	+'<td>'+currData[i].article_desc+'</td>';
			    	if(currData[i].article_up == "0"){
			    		dataHtml += '<td style="color:#f00">未置顶</td>';
			    	}else{
			    		dataHtml += '<td>置顶</td>';
			    	}
			    	dataHtml += '<td>'+currData[i].created_at+'</td>'
					+  '<td><a class="layui-btn layui-btn-mini news_edit" data-id="'+data[i].article_id+'"><i class="iconfont icon-edit"></i> 编辑</a>'
					+  '<a class="layui-btn layui-btn-danger layui-btn-mini news_del" data-id="'+data[i].article_id+'"><i class="layui-icon">&#xe640;</i> 删除</a>'
			        +'</td>'
			    	+'</tr>';
				}
			}else{
